...
import { test } from './tests/fixture/withFixture';
import withInfra from './tests/fixture/withInfra';
import withSchema from './tests/fixture/withSchema';
...
test.describe('extractTable', (test) => {
  withInfra(test, (test) => {
    withSchema(test, 'test', (test) => {
      test.it(
        'should extract simplified as well as full information_schema information',
        async ({ db }) => {
      ...
      test.it('should handle arrays', async ({ db }) => {
        await db.raw('create table test.some_table (id integer[])');
      ...
      withSchema(test, 'secondary_schema', (test) => {
        test.it('should extract a simple foreign key', async ({ db }) => {
        ...
        test.it(
          'should extract a foreign key with a different schema',
          async ({ db }) => {
            await db.raw(
        ...
        test.it(
          'should get the onDelete and onUpdate actions',
          async ({ db }) => {
            await db.raw('create table test.some_table (id int4 primary key)');
            await db.raw(
